...
def compile_shader(id, filename):
    ...
    if not glGetShaderiv(id, GL_COMPILE_STATUS):
        ...
        raise RuntimeError(f"Compilation error: {filename}\n{error}")


# Função para criar um programa e anexar shaders
def create_program(*shaders):
    id = glCreateProgram()
    if not id:
        raise RuntimeError("Could not create program")
...
# Função para linkar um programa
def link_program(id):
    glLinkProgram(id)
